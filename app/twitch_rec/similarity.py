...
class SimilarityScore:
    ...
    def __init__(self, mutual_followings: dict, total_followers: dict, n_best: int = 10) -> None:
...
class SorensenDiceSim(SimilarityScore):
    ...
    def sim(self, uid_mutual_count: int, live_uid_total_followers: int) -> float:
        result = -1
        if live_uid_total_followers + self.sum_mutual_followings != 0:
            result = (2 * uid_mutual_count) / (live_uid_total_followers + self.sum_mutual_followings)
...
class JaccardSim(SimilarityScore):
    ...
    def sim(self, uid_mutual_count: int, live_uid_total_followers: int) -> float:
        ...
        if live_uid_total_followers + uid_mutual_count != 0:
            result = uid_mutual_count / (self.sum_mutual_followings + live_uid_total_followers - uid_mutual_count)
...
def main():
    ...
    print('Sorensen-Dice')
    sd_sims = SorensenDiceSim(mutual_following_counts, uid_total_folls)
    print(f'  All Unsorted Sims: \n\t{sd_sims.sim_scores}')
    print(f'  N-best Ranked Sims: \n\t{sd_sims(n_best=None)}\n')

    print('Jaccard')
    j_sims = JaccardSim(mutual_following_counts, uid_total_folls)
    print(f'  All Unsorted Sims: \n\t{j_sims.sim_scores}')
    print(f'  N-best Ranked Sims: \n\t{j_sims(n_best=20)}\n')
