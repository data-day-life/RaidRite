...
class SimilarityScore:
    ...
    def sim_scores(self) -> dict:
        ...
        similarity_scores = {}
        for uid, tot_folls in self.sim_data.live_uid_tot_followers.items():
            similarity_scores.update(
                {uid: self.sim(self.sim_data.mutual_following_counts.get(uid, -1), tot_folls, sum_mutual_followings)})
...
def main():
    ...
    sim_data = SimData(mutual_following_counts, uid_total_folls)

    print('Sorensen-Dice')
    sd_sims = SorensenDiceSim(sim_data)
    print(f'  All Unsorted Sims: \n\t{sd_sims.sim_scores}')
    print(f'  N-best Ranked Sims: \n\t{sd_sims(n_best=None)}\n')

    print('Jaccard')
    j_sims = JaccardSim(sim_data)
    print(f'  All Unsorted Sims: \n\t{j_sims.sim_scores}')
    print(f'  N-best Ranked Sims: \n\t{j_sims(n_best=20)}\n')
